...
# Every /intelligence route now requires a valid analyst/admin token, so these
# HTTP tests run as an authenticated analyst. One register + login yields a
# bearer token attached to every request via the client default headers.
init_db()
_bootstrap = TestClient(app)
_bootstrap.post(
    "/auth/register",
    json={"username": "endpoint_tester", "password": "password99", "full_name": "Endpoint Tester"},
)
_TOKEN = _bootstrap.post(
    "/auth/login",
    json={"username": "endpoint_tester", "password": "password99"},
).json()["access_token"]
